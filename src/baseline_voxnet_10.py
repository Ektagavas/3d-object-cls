...
## VoxNet model with ModelNet10 dataset
...
class RandomRotate(object):
  ...
  def __call__(self, sample):
    ...
    #   rotated_data.append(rot_X)
    #   for i in range(n-1):
    #     rot_X = rotate(rot_X, axes=ax_tup, angle=angle, cval=0.0, reshape=False)
    #      # Thresholding
    #     rot_X = (rot_X >= 0.5).astype(int)
    #     rotated_data.append(rot_X)
    #   rot_X = np.array(rotated_data)
  ...
  for epoch in range(num_epochs):
      ...
      for phase in ['train', 'val']:
          ...
          for inputs, labels in dataloaders[phase]:
              ...
              # print(metrics.classification_report(labels.detach().cpu().numpy(), preds.detach().cpu().numpy(), digits=3))
      ...
      # if epoch == num_epochs - 1:
        # print(metrics.classification_report(labels.data, preds, digits=3))
...
plt.tight_layout()
# plt.show()
plt.savefig('/home/ektagavas/CV/training_plots_10_vox.png')
